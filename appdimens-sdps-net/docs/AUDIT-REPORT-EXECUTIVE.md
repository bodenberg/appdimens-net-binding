...
**Recommendation:** Published **1.0.0** (initial) and **1.0.1** (NuGet packaging); current MAUI release **3.6.0**. Android bindings remain on **3.5.1.4**. Document layout v2 and axis-neutral keys (`_N`) for anyone with hardcoded `Generated/sw/` paths.
